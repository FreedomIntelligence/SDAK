...
class ChatGLM2Worker(BaseWorker):
    ...
    def generate_fewshot_examples(self, data: list[dict], use_cot=False):
        ...
        for round, item in enumerate(data):
            ...
            prompt += (
                self.fewshot_template.format(round=round+1, user=user, gpt=gpt)
                + self.fewshot_separator
            )
        return prompt
    ...
    def collate_conv(self, data):
        ...
        for i, conv in enumerate(convs):
            if i == 0:
                ...
                line += f"[Round {i+1}]\n\n问：{user}\n\n"
                returned.append(
                    line.replace("[Round 1]\n\n", "") + "答："
                )  # do not include round information if there is only one question (no history)
            else:
                user = conv["question"]
                line += f"[Round {i+1}]\n\n问：{user}\n\n"
