...
def calculate_class_accuracy(all_class_dict, right_class_dict):
    ...
    for key in all_class_dict:
        accuracy_class_dict[key] = calculate_accuracy(
            all_class_dict[key], right_class_dict[key]
        )
    return accuracy_class_dict


def calculate_class_type_accuracy(all_class_type_dict, right_class_type_dict):
    accuracy_class_type_dict = defaultdict(
        lambda: defaultdict(lambda: defaultdict(int))
    )
    for key in all_class_type_dict:
        for inner_key in all_class_type_dict[key]:
            accuracy_class_type_dict[key][inner_key] = calculate_accuracy(
                all_class_type_dict[key][inner_key],
                right_class_type_dict[key][inner_key],
            )
    return accuracy_class_type_dict


def scorer(modelans):
    # with open(modelans_path, "r", encoding="utf-8") as file:
    ...
    for item in groundtruth:
        if "model_answer" in item:
            ...
            # if args.neglect_none_flag:
            #     # if not len(item["model_answer"]):
            #     if len(item["model_answer"].strip()) <= 0:
            #         continue
            all[exam_type] += 1
    ...
    accuracy_class_type_dict = calculate_class_type_accuracy(
        all_class_type, right_class_type
    )
...
if __name__ == "__main__":
    ...
    app.run(port=51234, debug=False) 
    groundtruth_path = 'CMB-test-choice-answer.json'
    # parser.add_argument(
    #     "--modelans_path",
    #     type=str,
