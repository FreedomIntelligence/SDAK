...
class HuatuoWorker(BaseWorker):
    ...
    def system_prompt(self):
        return "一位用户和智能医疗大模型Medflan-GPT之间的对话。对于用户的医疗问诊，Medflan-GPT给出准确的、详细的、温暖的指导建议。对于用户的指令问题，Medflan-GPT给出有益的、详细的、有礼貌的回答。"

    @property
    def instruction_template(
        self,
    ):
        return self.system_prompt + "<病人>：{instruction} <Medflan-GPT>："
    ...
    def instruction_template_with_fewshot(
        ...
        return (
            self.system_prompt + "{fewshot_examples}<病人>：{instruction} <Medflan-GPT>："
        )

    @property
    def fewshot_template(self, eos="</s>"):
        return "<病人>：{user} <Medflan-GPT>：{gpt}" + eos
    ...
    def collate_conv(self, data, eos="</s>"):
        ...
        for i, conv in enumerate(convs):
            ...
            returned.append(line + "<Medflan-GPT>：")
            ...
            line += "<Medflan-GPT>：{}".format(conv["solution"]) + eos
